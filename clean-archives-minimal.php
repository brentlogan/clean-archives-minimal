...
<?php
	...
	function PostList( $atts = array() ) {
		...
		if ( 1 == $atts['usejs'] ) $html .= '<a href="#" class="car-toggler button">' . __('Expand All', 'clean-archives-reloaded') . "</a>\n\n";
...
$html .= '			<li><a href="' . wp_make_link_relative(get_permalink( $post->ID )) . '">' .  mysql2date( 'd', $post->post_date ) . ' | ' . get_the_title( $post->ID ) . '</a>';
...
function init_CleanArchivesReloaded () {
	global $CleanArchivesReloaded; 
	$CleanArchivesReloaded = new CleanArchivesReloaded();
}
// add_action( 'init', init_CleanArchivesReloaded );
init_CleanArchivesReloaded ();
